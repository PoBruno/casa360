...
curl -X POST http://localhost:3000/api/auth/register \
  ...
  -d '{"name": "Test User", "email": "testuser@example.com", "password": "initialPass123"}'
echo -e "\n\n"
...
# Extraindo o token usando jq (certifique-se de ter jq instalado)
TOKEN=$(echo $LOGIN_RESPONSE | jq -r '.token')
echo "Token extraído: $TOKEN"
echo -e "\n"

# 3. Troca de senha
echo "Realizando troca de senha..."
# OBS.: O endpoint abaixo precisa estar implementado na API para troca de senha.
curl -X PUT http://localhost:3000/api/auth/changepassword \
  -H "Content-Type: application/json" \
  -H "Authorization: $TOKEN" \
  -d '{"oldPassword": "initialPass123", "newPassword": "newPass456"}'
echo -e "\n\n"

# 4. Login com a nova senha
echo "Realizando login com a nova senha..."
NEW_LOGIN_RESPONSE=$(curl -s -X POST http://localhost:3000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email": "testuser@example.com", "password": "newPass456"}')
echo "Resposta do novo login: $NEW_LOGIN_RESPONSE"
# Extraindo o novo token
NEW_TOKEN=$(echo $NEW_LOGIN_RESPONSE | jq -r '.token')
echo "Novo token extraído: $NEW_TOKEN"
echo -e "\n"

# 5. Consulta ao relatório financeiro (exemplo: lista de entradas financeiras)
echo "Consulta ao relatório financeiro..."
curl -X GET http://localhost:3000/api/financeEntries \
  -H "Authorization: $TOKEN"
echo -e "\n"
```
