...
export const createFinanceRecord = async (userId: number, financeCcId: number, financeCategoryId: number, financePayerId: number, startDate: Date, paymentDay: number, description: string, installmentsCount: number, isRecurring: boolean) => {
    const res = await query('INSERT INTO Finance_Entries (user_id, finance_cc_id, finance_category_id, finance_payer_id, start_date, payment_day, description, installments_count, is_recurring) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *', [userId, financeCcId, financeCategoryId, financePayerId, startDate, paymentDay, description, installmentsCount, isRecurring]);
    return res.rows[0];
};
